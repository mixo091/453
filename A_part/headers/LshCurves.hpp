...
class _HashTable{
    public:
      ...
      _HashTable(int size, int w, int k, int dim ,int _id,vector<Grid_Curve>* _snapped_data):Id(_id),Size(size) {
      ...
      void Print_snapped_data(){
        ...
        for (size_t i =0 ; i < snapped_data->size(); i++){
      ...
      void Print(){
        ...
        for(int b=0 ; b<Size ; b++){
          ...
          if(Buckets.at(b).BucketEntries.size()>0){
            cout<<"     [";
            for(size_t entry=0; entry < Buckets.at(b).BucketEntries.size() ; entry++){
      ...
      void FindNearestNeighbor(Grid_Curve* curve, map<double, int> &k_nearest_map, int k) 
      {
        ...
        if(Buckets.at(queryBucket).BucketEntries.size()>0){
          ...
          for(size_t i = 0 ; i < Buckets.at(queryBucket).BucketEntries.size() ; i++){
...
class Grid{
  public:
    ...
    Grid_Curve Curve_to_GridCurve(R2_Curve curve, bool query){
      ...
      vector<my_point> curve_points = curve.getVector();
      vector<my_point> snapped_points ;
      vector<double> result_vector ;
      // Snap each Dimension. //
      for(int i =0; i < num_of_points ; i++){
        my_point p = curve_points.at(i);
        double x_i = p.get_x();
        double y_i = p.get_y();
        ...
        my_point snapped_point(snapped_x ,snapped_y);
    ...
    void PrintSnappedDataSets(){
      ...
      for(size_t i = 0 ; i < snapped_queries.size() ; i++){
...
class Curve_Lsh{
    ...
    public:
        Curve_Lsh(int L, int total_curves,int _number_of_queries, int number_of_points, int _k, int _w,double _delta ,R2_Curve *data_set ,R2_Curve *_query_data_set, const string &out_file)
            ...
            w(_w),delta(_delta),input_data_set(data_set),query_data_set(_query_data_set),number_of_curves(total_curves),number_of_queries(_number_of_queries)
            {
              ...
              for(int query=0 ; query < number_of_queries ; query++){
               // auto start_lsh = high_resolution_clock::now();
                ...
                const auto end_DF = high_resolution_clock::now();
                auto duration_DF = duration_cast<microseconds>(end_DF - start_DF);
                ...
                string neighbhor_name = input_data_set[0].get_name();
                int neighbor_id = 0;
                for(int n = 0 ; n < number_of_curves ; n++){
                  if(min_true_distance > Discrete_Frechet(query_data_set[query].getVector(),input_data_set[n].getVector())){
                      ...
                      neighbhor_name = input_data_set[n].get_name();
                      neighbor_id = n;
                  }
                }
            ...
            output << "\n\ntApproximateAverage:    " << (double) (avg_duration_DF/ 1000000) / number_of_queries;
            output << "\ntTrueAverage:    " << (double) (avg_durationBF / 1000000) / number_of_queries; 
            output << "\nMAF:    " << endl << endl;
